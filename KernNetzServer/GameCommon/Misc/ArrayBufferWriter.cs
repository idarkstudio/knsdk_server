...
namespace FigNetCommon
{
    public class ArrayBufferWriter : IBufferWriter<byte>, IDisposable
    {
        ...
        private ArrayPool<byte> memoryPool = ArrayPool<byte>.Shared;
        private const int DefaultBufferSize = 1024;
        public byte[] WrittenBytes;
        public void PrepareWrittenBytes()
        {
            WrittenBytes = memoryPool.Rent(_written);
            Buffer.BlockCopy(_rentedBuffer, 0, WrittenBytes, 0, _written);
        }

        public void RecycleWrittenBytes()
        {
            if (WrittenBytes != null) memoryPool.Return(WrittenBytes);
        }


        public ArrayBufferWriter(int initialCapacity = DefaultBufferSize)
        {
            if (initialCapacity <= 0)
                throw new ArgumentException(nameof(initialCapacity));

            _rentedBuffer = memoryPool.Rent(initialCapacity);
            _written = 0;
            _committed = 0;
        }
        public ArraySegment<byte> GetAsArraySegment()
        {
            return new ArraySegment<byte>(WrittenBytes, 0, _written);
        }
